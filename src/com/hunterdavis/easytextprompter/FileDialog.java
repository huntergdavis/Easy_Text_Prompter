...
public class FileDialog extends ListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		 ...
		 * 
		 * mFileName.setText(""); mFileName.requestFocus(); } })0
		 */
	...
	private void getDirImpl(String dirPath) {
		...
		for (File file : files) {
			if (file.isDirectory()) {
				...
				String ourPath = file.getPath();
				int dotPos = ourPath.lastIndexOf(".");
				if (dotPos == -1) {
					foundIt = true;
				} else {
					String ourextention = ourPath.substring(dotPos);

					if (ourextention.equalsIgnoreCase(".txt")) {
						foundIt = true;
					}
				}
				if (foundIt) {
